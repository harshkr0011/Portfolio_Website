...
const Footer = () => {
  ...
  return (
    <footer className="relative bg-background/50 backdrop-filter backdrop-blur-lg border-t border-white/10 mt-8">
      <div className="container mx-auto px-4 py-6 md:px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-center md:text-left">
          ...
          <div className="flex flex-col items-center">
            <h3 className="font-headline text-base font-semibold mb-3 text-center w-full">Navigate</h3>
            <ul className="grid grid-cols-2 gap-x-6 gap-y-2 w-full justify-items-center">
              {navItems.map((item, index) => {
                 const LinkComponent = item.href.startsWith('/') || item.href.startsWith('#') ? Link : 'a';
                 return (
                    <li key={index}>
                      <LinkComponent
                        href={item.href}
                        className="text-sm text-muted-foreground transition-colors hover:text-primary block py-1"
                      >
                        {item.label}
                      </LinkComponent>
                    </li>
          ...
          <div className="flex flex-col items-center md:items-end">
             <h3 className="font-headline text-base font-semibold mb-2">Get In Touch</h3>
            <div className="flex items-center gap-2 text-xs text-muted-foreground hover:text-primary transition-colors mb-2">
        ...
        <motion.div
            ...
            className="mt-6 pt-4 border-t border-white/10 text-center text-xs text-muted-foreground"
        >
